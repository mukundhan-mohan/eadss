...
from app.core.admin_auth import verify_password, create_token, set_session_cookie, clear_session_cookie, require_admin, hash_password
...
def register(payload: AdminRegisterIn, resp: Response, db: Session = Depends(get_db)):
    ...
    user = AdminUser(email=email, password_hash=hash_password(payload.password), is_active=True)
    db.add(user)
    db.commit()
    db.refresh(user)
    ...
    return AdminMeOut(id=str(user.id), email=user.email)
